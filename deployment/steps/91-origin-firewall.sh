...
SELF='/opt/mte-platform/steps/91-origin-firewall.sh'
...
require_tools() {
  local tool
  for tool in awk flock ip iptables ip6tables python3 sha256sum stat systemctl; do
  ...
  "$tool" -w -S "$base" | awk -v base="$base" -v marker="$comment" '
    $1 == "-A" && $2 == base {
      for (index = 1; index <= NF; index++) {
        if ($index == "--comment" && $(index + 1) == marker) print $0
      }
    }
  '
}
...
jump_target() {
  awk '{for (index = 1; index <= NF; index++) if ($index == "-j") print $(index + 1)}' <<<"$1"
}
...
status_rules() {
  local interface_v4 interface_v6 normalized fingerprint active=false enabled=false
  load_cidrs
  interface_v4=$(public_interface_v4)
  interface_v6=$(public_interface_v6)
  validate_interface "$interface_v4"
...
case "$ACTION" in
  ...
  remove)
    ...
    ;;
  *)
    echo 'usage: 91-origin-firewall.sh apply|enforce|recover|status|remove' >&2
    exit 2
    ;;
esac
